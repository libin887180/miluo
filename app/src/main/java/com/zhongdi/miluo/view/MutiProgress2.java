 ...
 * @author huqiang
 */
public class MutiProgress2 extends View {
    ...
    public MutiProgress2(Context context) {
        this(context, null);

    }

    public MutiProgress2(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MutiProgress2(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        unprogressingDrawable = mTypedArray.getDrawable(R.styleable
                .MutiProgress_unprogressingDrawable);
        ...
        processingLineColor = mTypedArray.getColor(R.styleable.MutiProgress_processingLineColor,
                DEFAULT_LINE_COLOR);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        nodes = new ArrayList<MutiProgress2.Node>();
        ...
        for (int i = 0; i < nodesNum; i++) {
            ...
            if (i == 0)
                ...
                node.mPoint = new Point(((int) nodeWidth * i) - nodeRadius * 2, mHeight / 2 -
                        nodeRadius);
            else
                node.mPoint = new Point(((int) nodeWidth * i) - nodeRadius, mHeight / 2 -
                        nodeRadius);
            if (currNodeNO == i)
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mCanvas.drawLine(0, nodes.get(currNodeNO)
                .mPoint.y + nodeRadius, mWidth, mHeight / 2, mPaint);
        ...
        for (int i = 0; i < nodes.size(); i++) {
            ...
            {
                progressingDrawable.setBounds(node.mPoint.x, node.mPoint.y, node.mPoint.x +
                        nodeRadius * 2, node.mPoint.y + nodeRadius * 2);
                progressingDrawable.draw(canvas);
            } else if (i == currNodeNO)  //当前所到达的进度节点（终点）
            {
                if (currNodeState == 1) //判断是成功还是失败  0 :失败  1：成功
                {
                    progresSuccDrawable.setBounds(node.mPoint.x, node.mPoint.y, node.mPoint.x +
                            nodeRadius * 2, node.mPoint.y + nodeRadius * 2);
                    progresSuccDrawable.draw(canvas);
                } else {
                    progresFailDrawable.setBounds(node.mPoint.x, node.mPoint.y, node.mPoint.x +
                            nodeRadius * 2, node.mPoint.y + nodeRadius * 2);
                    progresFailDrawable.draw(canvas);
                }
            } else   //未完成的进度节点
            {
                unprogressingDrawable.setBounds(node.mPoint.x, node.mPoint.y, node.mPoint.x +
                        nodeRadius * 2, node.mPoint.y + nodeRadius * 2);
    ...
    private void DrawProgerss() {
        if (progress > 0) {
            mPaint.setColor(processingLineColor);   //先画线段，线段的高度为nodeRadius/2
            mPaint.setStrokeWidth(nodeRadius / 2);
            mCanvas.drawLine(0, mHeight / 2, progress, nodes.get(currNodeNO).mPoint.y +
                    nodeRadius, mPaint);
        }
    }
